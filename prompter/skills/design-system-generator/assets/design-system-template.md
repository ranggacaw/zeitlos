...
<!-- Remove if all fonts are system fonts -->

| Family | Source | Weights Loaded | Fallback Stack |
|--------|--------|----------------|----------------|
| `{{FAMILY}}` | {{self-hosted / Google Fonts / system}} | `{{WEIGHTS}}` | `{{FULL_STACK}}` |
...
| `--ds-text-5xl` | `{{PX}}` | `{{REM}}` | Large hero text |
| `--ds-text-6xl` | `{{PX}}` | `{{REM}}` | Display text |
| `--ds-text-7xl` | `{{PX}}` | `{{REM}}` | Oversized display |

<!-- Remove size rows not found in the source -->

### Font Weights

<!-- Include only weights actually found in the source; remove unused rows -->

| Token | Value | Usage |
|-------|-------|-------|
| `--ds-font-light` | `{{VALUE}}` | Light emphasis |
| `--ds-font-normal` | `{{VALUE}}` | Body text |
| `--ds-font-medium` | `{{VALUE}}` | Subtle emphasis |
| `--ds-font-semibold` | `{{VALUE}}` | Strong emphasis |
| `--ds-font-bold` | `{{VALUE}}` | Headings, CTAs |
