...
### Font loading & provenance

Record WHERE each family comes from, not just its name:
- `@font-face` blocks → capture `font-family`, `src` (file/format), `font-weight` range,
  `font-display`, and variable-font axes if present
- `<link href="https://fonts.googleapis.com/...">` → note the Google Fonts URL and requested weights
- `next/font` / `@fontsource/*` imports → note package + subsets
- Always record the **full fallback stack** verbatim (e.g. `"Inter", ui-sans-serif, system-ui, sans-serif`)

Document in the typography token page as a "Font sources" table:
`family · source (self-hosted / Google Fonts / system) · weights loaded · fallback stack`.

### Line height, letter spacing, text transform

Target: `line-height`, `letter-spacing`, `text-transform`, `--leading-*`, `--tracking-*`

- Line heights → map to `tight` (≤1.25), `normal` (1.3–1.6), `relaxed` (≥1.6); keep unitless when source is unitless
- Letter spacing → `tight` (negative), `normal` (0), `wide` (positive); note uppercase-label pairings
- Text transform → record recurring patterns (e.g. uppercase + wide tracking on labels/buttons)
